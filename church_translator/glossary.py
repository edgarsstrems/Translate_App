...
BUILTIN_SERMON_REPLACEMENTS: list[tuple[re.Pattern, str]] = [
    ...
    (re.compile(r"\bjezus\s+v[aā]rd[aā]\b", re.IGNORECASE), "Jēzus vārdā"),
    (re.compile(r"\bkunga\s+v[aā]rd[aā]\b", re.IGNORECASE), "Kunga vārdā"),
    (re.compile(r"\bdieva\s+v[aā]rd[aā]\b", re.IGNORECASE), "Dieva vārdā"),
    (re.compile(r"\bmūsu\s+kungs\b", re.IGNORECASE), "mūsu Kungs"),
    (re.compile(r"\bmūsu\s+kunga\b", re.IGNORECASE), "mūsu Kunga"),
    (re.compile(r"\bmūsu\s+kungam\b", re.IGNORECASE), "mūsu Kungam"),
    (re.compile(r"\bbibel[eē]\b", re.IGNORECASE), "Bībelē"),
    (re.compile(r"\bbibeli\b", re.IGNORECASE), "Bībeli"),
    (re.compile(r"\bbibele\b", re.IGNORECASE), "Bībele"),
    (re.compile(r"\bmuž[iī]g[aā]\s+dz[iī]v[iī]b[a-zāēīūļķņģčšž]*\b", re.IGNORECASE), "mūžīgā dzīvība"),
    (re.compile(r"\bzelast[iī]b[a-zāēīūļķņģčšž]*\b", re.IGNORECASE), "žēlastība"),
    (re.compile(r"\bpest[iī]san[a-zāēīūļķņģčšž]*\b", re.IGNORECASE), "pestīšana"),
    (re.compile(r"\b(un|mēs|ka|tas|jo|lai|bet)\s+\1\b", re.IGNORECASE), r"\1"),
